...
The Metadata Flags (reversible and irreversible) are reserved for bitwise operations
with a bitmask.
...
> The **MainNet** `netauth` is empty, therefore, the _partial_ Asset URL on MainNet is:
...
> resulting in the following _partial_ Asset URL:
...
If the Deprecated By field is not `0`, clients **SHALL** point to the new [ARC-90](/arc-standards/arc-0090)
Asset Metadata URI:
...
the [Asset Metadata URI](#asset-metadata-uri) as [ARC-2](/arc-standards/arc-0002) `AssetConfig`
...
> The [ARC-2](/arc-standards/arc-0002) note to migrate existing [ARC-3](/arc-standards/arc-0003) ASA
